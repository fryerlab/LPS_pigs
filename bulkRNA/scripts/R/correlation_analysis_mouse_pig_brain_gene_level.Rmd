...
subset(mouse_limma_FDRq1.00_orth, ortholog_name == "BEX2")
subset(pig_brain_FDRq1.00_orth, ortholog_name == "BEX2")

pig_gene_name_dups <- human_pig_orth[duplicated(human_pig_orth$ortholog_name),]
pig_gene_name_dups_unique <- unique(pig_gene_name_dups$ortholog_name)

mouse_gene_name_dups <- human_mouse_orth[duplicated(human_mouse_orth$ortholog_name),]
mouse_gene_name_dups_unique <- unique(mouse_gene_name_dups$ortholog_name)

library(gtools)
test <-
  merge(mouse_limma_FDRq1.00_orth, pig_brain_FDRq1.00_orth, by = "ortholog_name",  all=T)
```
...
shared_t <-
  test[test$ortholog_name %in% shared_gene_name,]
uniqueToMouse_t <-
  test[test$ortholog_name %in% uniqueToMouse_gene_name,]
uniqueToPig_t <-
  test[test$ortholog_name %in% uniqueToPig_gene_name,]
testtest <- subset(uniqueToPig_t, gene_name != "NA")
