...
May need to unload all packages 
...
up10 <- up[1:5, ]
up10 <- up10[!grepl('ENSSSCG',up10$gene_name),]

upFoldChange <-
  treatment_vs_control[treatment_vs_control$logFC > 7,]
upFoldChange <- upFoldChange[5:32, ]
...
downFoldChange <-
  treatment_vs_control[treatment_vs_control$logFC < -5 &
                         treatment_vs_control$adj.P.Val < 0.05, ]
#downFoldChange <- downFoldChange[5:15, ]
downFoldChange <- downFoldChange[!grepl('ENSSSCG',downFoldChange$gene_name),]

goi <- 
  treatment_vs_control[treatment_vs_control$gene_name == "TMEM225B",]
goi2 <- 
  treatment_vs_control[treatment_vs_control$gene_name == "P4HA2",]
goi3 <- 
  treatment_vs_control[treatment_vs_control$gene_name == "ADAMTS12",]

goi4 <- 
  treatment_vs_control[treatment_vs_control$gene_name == "YPEL4",]
goi5 <- 
  treatment_vs_control[treatment_vs_control$gene_name == "ATP4B",]
goi6 <- 
  treatment_vs_control[treatment_vs_control$gene_name == "HNMT",]
...
up_and_down_enrich_results_subset$Description <-
      gsub(
        ...
        "reg protein modification by conjugation or removal",
        up_and_down_enrich_results_subset$Description
      )
up_and_down_enrich_results_subset$Description <-
      gsub(
...
## up 
 ...
 [5] cellular protein catabolic process                                        
 [6] regulation of cell cycle process                                          
 [7] cellular response to DNA damage stimulus                                  
 [8] regulation of DNA metabolic process                                       
 [9] Cell Cycle Checkpoints                                                    
[10] response to virus                                                         
[11] Signaling by ROBO receptors                                               
[12] regulation of chromosome organization                                     
[13] regulation of protein catabolic process                                   
[14] Signaling by Rho GTPases, Miro GTPases and RHOBTB3                        
[15] positive regulation of cell cycle                                         
[16] Interferon Signaling                                                      
[17] immune system development                                                 
[18] Apoptosis                                                                 
[19] reg protein modification by conjugation or removal
[20] Transcriptional Regulation by TP53   
```{r}
up_enrich_df <-
...
up_input <- list(
  "Cytokine Signaling in Immune system" = up_gene_char[[2]],
  "Cellular responses to stress" = up_gene_char[[4]],
  "Cell Cycle Checkpoints" = up_gene_char[[9]],
  "Signaling by Rho/Miro GTPases & RHOBTB3" = up_gene_char[[14]],
  "Apoptosis" = up_gene_char[[18]] 
)
...
GO_2 <-
  subset(up_df,
         up_df$variable == "Cellular responses to stress"
         & up_df$value == 1)
GO_2_top <- head(GO_2, 15)

GO_3 <-
  subset(up_df,
         up_df$variable == "Cell Cycle Checkpoints"
         & up_df$value == 1)
GO_3_top <- head(GO_3, 15)

GO_4 <-
  subset(up_df,
         up_df$variable == "Signaling by Rho/Miro GTPases & RHOBTB3"
         & up_df$value == 1)
GO_4_top <- head(GO_4, 15)

GO_5 <-
  subset(up_df,
         up_df$variable == "Apoptosis"
         & up_df$value == 1)
...
## down
 ...
 [3] Hemostasis                               
 [4] actin cytoskeleton organization          
 [5] regulation of vesicle-mediated transport 
 [6] carbohydrate metabolic process           
 [7] Lysosome                                 
 [8] regulation of cellular localization      
 [9] TYROBP causal network in microglia       
[10] Membrane Trafficking                     
[11] negative regulation of catalytic activity
[12] Signaling by Receptor Tyrosine Kinases   
[13] cell activation                          
[14] positive regulation of hydrolase activity
[15] Salmonella infection                     
[16] PID RAC1 PATHWAY                         
[17] inflammatory response                    
[18] vacuole organization                     
[19] myeloid cell differentiation             
[20] small GTPase mediated signal transduction
```{r}
down_enrich_df <-
...
down_input <- list(
  "Neutrophil degranulation" = down_gene_char[[1]],
  "Signaling by Rho GTPases" = down_gene_char[[2]],
  "regulation of vesicle-mediated transport" = down_gene_char[[5]], 
  "inflammatory response" = down_gene_char[[17]],
  "small GTPase mediated signal transduction" = down_gene_char[[20]]
)
...
GO_1 <-
  subset(down_df,
         down_df$variable == "Neutrophil degranulation"
         & down_df$value == 1)
GO_1_top <- head(GO_1, 15)

GO_2 <-
  subset(down_df,
         down_df$variable == "Signaling by Rho GTPases"
         & down_df$value == 1)
GO_2_top <- head(GO_2, 15)

GO_3 <-
  subset(down_df,
         down_df$variable == "regulation of vesicle-mediated transport"
         & down_df$value == 1)
GO_3_top <- head(GO_3, 15)

GO_4 <-
  subset(down_df,
         down_df$variable == "inflammatory response"
         & down_df$value == 1)
GO_4_top <- head(GO_4, 15)

GO_5 <-
  subset(down_df,
         down_df$variable == "small GTPase mediated signal transduction"
         & down_df$value == 1)
GO_5_top <- head(GO_5, 15)
```
#### plot lcpm GO heatmaps
