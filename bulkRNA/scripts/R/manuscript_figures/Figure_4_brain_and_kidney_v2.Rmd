...
up_and_down_enrich_plot <-
  ...
  scale_fill_gradientn(
    ...
    theme(strip.text = element_text(size = 6), 
          ...
          legend.box.margin=margin(-10,-2,-10,-7.5), 
          plot.margin = margin(0.1, 0.2, 0, 0.2, "cm"), 
          plot.title = element_text(size = 7, hjust = 7.5, vjust=0, margin = margin(0,0,1,0)))
up_and_down_enrich_plot
...
row4 <- ggarrange(
  ...
  nrow = 1, 
  ncol = 8,
  labels = c("h)", "i)", "j)", "k)", "h)", "i)", "j)", "k)"),
  font.label = list(size = 8)
)


together <- ggarrange(row1, row2, row3, row4, nrow = 4, heights = c(0.875, 1.2, 0.125, 1.15))
together
