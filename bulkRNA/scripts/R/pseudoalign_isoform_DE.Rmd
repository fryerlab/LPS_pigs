...
# rename columns
names(isoform_count_table)[1] <- "gene_name"
names(isoform_count_table)[2] <- "isoforms"
names(isoform_count_table)[3] <- "isoforms_qval0.05"
# difference between total number of isoforms and isoforms of that gene called as DE, qval < 0.05
isoform_count_table$diff <-
  isoform_count_table$isoforms - isoform_count_table$isoforms_qval0.05

# genes that have multiple isoforms but not all isoforms are differential expressed, qval < 0.05
isoform_bias <- subset(isoform_count_table, diff > 0)

# merge  sleuth table with isoform count table to get all transcripts of the DE genes
isoform_bias_table <-
  merge(sleuth_table_wt, isoform_count_table, by = "gene_name")
...
```{r plot_isoform_expression}
# all isoforms of the genes that are DE
# this is the same as the isoform_bias_table 
all_iso_sig_genes <- sleuth_table_wt[sleuth_table_wt$gene_name %in% sleuth_significant$gene_name, ]

path <- paste0("../../results/", tool, "/boxplot/", treatment, "_",tolower(tissue),"_isoforms")
pdf(paste0(path, ".pdf"), width = 4, height = 4)

# visualize both biological and technical variation for selected transcripts:
# may use tpm or est_counts 
violin_Func <- function(a, b){
  iso <- plot_bootstrap(so, target_id = a, units = "tpm", color_by = "condition") + 
    theme(legend.position = "none") + 
    scale_fill_manual(values = c(control_color, treatment_color)) + 
    labs(title = b, subtitle = a)
  iso
}
# run the violin_Func
violinPlots <- Map(violin_Func, a = all_iso_sig_genes$target_id, b = all_iso_sig_genes$gene_name)
violinPlots
dev.off()
```
